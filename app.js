...
const DOMManager = (() => {
  ...
  // Names of PIDs queued for auto-selection once they appear in AppStore
  // (needed because in cloud mode ids arrive asynchronously via onSnapshot).
  const pendingSelectPidNames = new Set();
  function drainPendingPidSelections(){
    if(!pendingSelectPidNames.size) return false;
    let changed = false;
    for(const name of [...pendingSelectPidNames]){
      const p = AppStore.state.pids.find(x=>x.name===name);
      if(p){ AppStore.togglePid(p.id, true); pendingSelectPidNames.delete(name); changed = true; }
    }
    return changed;
  }
  ...
  function bindIntegration(){
    ...
    // Normalize multi-line / tab-separated pastes (e.g. from Google Sheets) into comma-separated tokens
    $('#pidInput').addEventListener('paste', ev => {
      const cd = ev.clipboardData; if(!cd) return;
      const text = cd.getData('text'); if(!text) return;
      if(!/[\n\r\t;]/.test(text)) return; // let normal paste through
      ev.preventDefault();
      const tokens = text.split(/[,\n\r\t;]+/).map(s=>s.trim()).filter(Boolean);
      const inp = ev.target;
      const before = inp.value.slice(0, inp.selectionStart||0);
      const after  = inp.value.slice(inp.selectionEnd||0);
    ...
    $('#addPidForm').addEventListener('submit', async e=>{
      e.preventDefault();
      const raw = $('#pidInput').value;
      const tokens = [...new Set(raw.split(/[,\n\t;]+/).map(s=>s.trim()).filter(Boolean))];
      const pubs = $('#pidPubs').value.split(',').map(s=>s.trim()).filter(Boolean);
      if(!tokens.length){ toast('Enter at least one PID', true); return; }

      // Single-token legacy behavior: existing PID with no new pubs -> nudge user
      if(tokens.length===1 && !pubs.length){
        const existing = AppStore.state.pids.find(p=>p.name===tokens[0]);
        if(existing){
          AppStore.togglePid(existing.id, true);
          renderAssets();
          toast('That PID already exists.', true);
          $('#pidInput').value=''; $('#pidPubs').value='';
          return;
        }
      }
      ...
      for(const name of tokens){
        const existing = AppStore.state.pids.find(p=>p.name===name);
        try{
          if(!existing){
            await CloudSync.addPid(name, pubs);
            added++;
          } else {
            for(const pub of pubs){
              if(!(existing.pubNames||[]).includes(pub)){
                await CloudSync.addPubToPid(existing.id, pub);
              }
            }
            AppStore.togglePid(existing.id, true);
            merged++;
          }
          pendingSelectPidNames.add(name);
        }catch(err){ failed++; console.error('Add PID failed:', name, err); }
      }
      $('#pidInput').value=''; $('#pidPubs').value='';
      drainPendingPidSelections();
      renderAssets();
      toast(`PIDs — added ${added}, merged ${merged}${failed?`, failed ${failed}`:''}`, failed>0);
    });
  ...
  function afterPids(){ drainPendingPidSelections(); renderAssets(); }
